...
impl EfiBootServicesTable {
    fn get_mamory_map(&self, map: &mut MemoryMapHolder) -> EfiStatus {
        (self.get_memory_map)(
...
fn efi_main(image_handler: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    .unwrap();
    exit_boot_services(image_handler, efi_system_table, &mut memory_map);
...
fn fill_rect<T: Bitmap>(buf: &mut T, color: u32, px: i64, py: i64, w: i64, h: i64) -> Result<()> {
    ...
    for dy in 0..h {
        for dx in 0..w {
            unsafe {
                unchecked_draw_point(buf, color, px + dx, py + dy);
            }
        }
    }
    Ok(())
...
fn calc_slope_point(da: i64, db: i64, ia: i64) -> Option<i64> {
    // return the corresponding coordinate on the slower axis for given ia on the faster axis
    if da < db {
        None
    } else if da == 0 {
        Some(0)
